...
export default function PccPage() {
  ...
  async function addPcc() {
    if (!form.zoneId) return toast.error("Select a Plant Main Input first");
    if (!form.name) return toast.error("PCC panel name required");
    ...
    if (capturedPhoto && capturedPhoto.startsWith("data:")) {
      try {
        const cleanPccName = sanitizeName(form.name) || "pcc";
        const dateStr = getFormattedDate();
        const fileName = `${cleanPccName}_${dateStr}`;
    ...
    if (editingId) {
      ...
      toast.success("PCC Panel updated locally");
    } else {
      addAreaAction(payload);
      toast.success("PCC Panel added locally");
    }
  ...
  function handleDelete(id: string) {
    if (confirm("Are you sure you want to delete this PCC Panel? All connected motor load data will also be deleted.")) {
      deleteAreaAction(id);
      toast.success("PCC Panel deleted");
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader>
          <CardTitle>{editingId ? "Edit PCC Panel" : "Add PCC Panel"}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-3 md:grid-cols-4">
            ...
            <div>
              <Label>PCC Panel Name</Label>
              <Input value={form.name} onChange={(e) => setForm({...form, name: e.target.value})} placeholder="e.g. PCC-1" />
            </div>
            <div>
              <Label>PQ Name</Label>
              <select 
          ...
          <div className="flex items-end gap-2 pt-2">
            <Button onClick={() => void addPcc()}>{editingId ? "Update Entry" : "Add Entry"}</Button>
      ...
      <Card>
        <CardHeader>
          <CardTitle>PCC Panels Recorded</CardTitle>
        </CardHeader>
        <CardContent>
          {pccPanels.length > 0 ? (
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="text-slate-400 border-b border-white/10">
                    ...
                    <th className="text-left px-2 py-2">PCC Name</th>
            ...
            <p className="text-slate-400 text-sm">No PCC panels recorded yet.</p>
          )}
        </CardContent>
      </Card>
    </div>
